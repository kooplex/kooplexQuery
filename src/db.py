...
WHERE c.relkind in ('r', 'v') AND n.nspname = :schema AND c.relname LIKE :table_name_like
ORDER BY c.relname;
            """
            r=self.query(q, {'schema': self.schema, 'table_name_like': table_name_like})
        else:
            q="""
SELECT c.relname AS table_name, obj_description(c.oid) AS table_comment
FROM pg_class c
...
WHERE c.relkind in ('r', 'v') AND n.nspname = :schema AND a.attnum > 0 AND NOT a.attisdropped AND c.relname LIKE :table_name_like
